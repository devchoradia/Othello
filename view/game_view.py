...
from view.abstract_page_view import PageView, BUTTON_FONT, APP_COLOR, ROW_HEIGHT
...
class GameView(PageView):
    ...
    def add_current_player(self, player=Player.BLACK):
        ...
        label = tk.Label(frame, borderwidth=2, font=BUTTON_FONT, height=ROW_HEIGHT,
                         text=f"Your turn: {PLAYER_COLOR[player]}", bg='white', fg="black")
    ...
    def add_navigator(self):
        if self.on_restart is None:
            ...
            left_frame = tk.Frame(self, borderwidth=0, bg=APP_COLOR, highlightbackground=APP_COLOR,
                                  highlightcolor=APP_COLOR)
            left_frame.pack(expand=True, fill=tk.BOTH, side=tk.LEFT)
            label = tk.Button(left_frame, text="Restart", borderwidth=1, height=ROW_HEIGHT, command=self.on_restart,
                              bg=APP_COLOR, highlightbackground=APP_COLOR)
            label.pack(side=tk.RIGHT, padx=10, pady=10)
            right_frame = tk.Frame(self, borderwidth=0, bg=APP_COLOR, highlightbackground=APP_COLOR,
                                   highlightcolor=APP_COLOR)

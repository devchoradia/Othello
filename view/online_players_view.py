...
class OnlinePlayersView(GridPageView):
    ...
    def display(self):
        ...
        for row, player in enumerate(self.players):
            ...
            for col in range(2):
                ...
                tile = tk.Label(frame, borderwidth=1, width=COLUMN_WIDTHS[col], height=ROW_HEIGHT,
            ...
            tile = tk.Button(frame, font=PLAY_BUTTON_FONT, text="Play", borderwidth=2, height=ROW_HEIGHT,
                             command=command, state=clickable, bg="white", fg="black", highlightbackground="white")
    ...
    def add_no_online_players_message(self):
        ...
        frame = tk.Frame(self, padx=5, pady=5, relief=tk.RAISED, borderwidth=0, bg=APP_COLOR)
        frame.grid(row=grid_row, columnspan=len(COLUMNS), sticky=STICKY)
        tile = tk.Label(frame, borderwidth=0, height=ROW_HEIGHT, text="No other online players available", bg=APP_COLOR,
                        fg="black")
        tile.pack(ipadx=1, ipady=1, expand=True)
